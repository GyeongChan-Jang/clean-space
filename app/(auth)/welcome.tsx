  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-white">
      ...
      <Swiper
        ...
        activeDot={<View className="w-[32px] h-[4px] mx-1 bg-[#5CCEF8] rounded-full" />}
        ...
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <View className="flex flex-row items-center justify-center w-full mt-10">
              <Text className="text-black text-2xl font-bold mx-10 text-center">{item.title}</Text>
            </View>
            <Text className="text-md font-PretendardBold text-center text-[#858585] mx-10 mt-3">
              {item.description}
            </Text>
          </View>
        ))}
