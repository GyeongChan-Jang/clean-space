      ...
      <ScrollView className="flex-1 px-6">
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <View className="mb-6">
              <Text className="text-xl font-PretendardSemiBold mb-2">
                공간 이름
              </Text>
              <TextInput
                onBlur={onBlur}
            ...
            <View className="mb-6">
              ...
              <TextInput
                ...
                className={`text-lg p-4 border rounded-md text-gray-900 font-Pretendard h-[120px] ${
                  errors.description
                    ? "border-danger-800"
                    : "border-secondary-600 focus:border-black"
                }`}
              />
        ...
        <Controller
          ...
          rules={{
            ...
            validate: (value) =>
              Number(value) > 0 || "0보다 큰 숫자를 입력해주세요",
          }}
          render={({ field: { onChange, onBlur, value } }) => (
            <View className="mb-6">
              <Text className="text-xl font-PretendardSemiBold mb-2">
              ...
              />
              {errors.size && (
                <Text className="text-red-500 mt-1">{errors.size.message}</Text>
              )}
        ...
        <View className="mb-6">
          {[
            ...
            >
              <Text className="text-lg font-Pretendard text-secondary-900">
                {item.label}
              </Text>
              <View className="flex-row items-center">
                <TouchableOpacity
                  onPress={() => handleDecrement(item.field)}
                  className="p-2"
                >
                  <Ionicons
                ...
                <Text className="mx-2 text-xl font-Pretendard">
                  {spaceDetails[item.field]}
                </Text>
                <TouchableOpacity
                  onPress={() => handleIncrement(item.field)}
                  className="p-2"
                >
                  <Ionicons
